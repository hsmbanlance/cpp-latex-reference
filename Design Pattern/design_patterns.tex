...
如果项目中确实存在多个必须使用单例的场景，可以用 CRTP 提取公共样板（\texttt{\seqsplit{instance()}}、删除拷贝/移动），避免每个单例类重复编写：
...
C++ 标准库的迭代器体系是迭代器模式最完善的实现。自定义容器只需提供 \texttt{\seqsplit{begin()}}/\texttt{\seqsplit{end()}} 方法即可与所有标准算法兼容：
...
文件系统遍历 & 递归遍历目录树中的所有文件（如 \texttt{\seqsplit{std::filesystem::recursive\_directory\_iterator}}） \\
...
Composite & 结构型 & \texttt{\seqsplit{vector<unique\_ptr>}} & C.35, R.20 & 文件系统、AST \\
