...
本节是全文的核心：给出系统性的方法，将 C++20 的 Concepts 改写为 C++11/14/17 可用的 SFINAE 代码。
...
以下演示如何将 \texttt{std::random\_access\_iterator} 降级到 C++17 SFINAE：
...
\midrule
成员函数方式 & \texttt{template<class Ar> serialize(Ar\&)} & \texttt{serialize(Ar\&, unsigned int)} \\
自由函数方式 & \texttt{template<class Ar> serialize(Ar\&, T\&)} & \texttt{serialize(Ar\&, T\&, unsigned int)} \\
归档类型 & 模板参数（泛型） & 具体归档类型 \\
...
\begin{enumerate}
  ...
  \begin{itemize}[nosep]
    ...
    \item 操作要求（\texttt{requires \{ expr; \}}）$\to$ 定义 \texttt{has\_xxx} trait，用 \texttt{void\_t} + \texttt{decltype}。
    \item 嵌套要求（\texttt{requires bool-expr;}）$\to$ 直接写成布尔条件。
  \end{itemize}
  \item \textbf{处理返回值约束}
  \begin{itemize}[nosep]
    \item \texttt{\{ expr \} -> std::same\_as<U>} $\to$ \texttt{std::is\_same<decltype(expr), U>}。
    \item \texttt{\{ expr \} -> std::convertible\_to<U>} $\to$ \texttt{std::is\_convertible<decltype(expr), U>}。
  \end{itemize}
  \item \textbf{组合多个约束}
  \begin{itemize}[nosep]
    \item \texttt{concept\_a<T> \&\& concept\_b<T>} $\to$ \texttt{enable\_if\_t<trait\_a<T>::value \&\& trait\_b<T>::value>}。
