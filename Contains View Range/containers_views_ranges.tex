...
\cpp{17} 引入了 \Tp{\seqsplit{std::pmr::polymorphic\_allocator}}，通过
\Tp{\seqsplit{std::pmr::memory\_resource}} 实现运行时多态分配策略，避免模板参数传染：
...
% ────────────────────────────────────────────────────────────
\chapter[有序关联容器：map, set, multimap, multiset]{有序关联容器：\\map,\allowbreak{} set,\allowbreak{} multimap,\allowbreak{} multiset}
% ────────────────────────────────────────────────────────────
...
C++ 的 \Tp{\seqsplit{std::basic\_string}} 是一个完整的容器，支持迭代器、\Fn{size}、
\Tp{\seqsplit{operator[]}} 等容器接口。但由于字符串在编程中的特殊地位，标准库对其进行了
...
\cpp{20} 引入了 \Fn{std::format}，\cpp{23} 引入了 \Fn{std::print} / \Fn{\seqsplit{std::println}}：
