...
\texttt{join()}阻塞调用线程，直到目标线程执行完毕。\texttt{detach()}将目标线程与\texttt{\seqsplit{std::thread}}对象分离，使其成为守护线程（daemon thread），在后台独立运行。
...
\texttt{\seqsplit{std::thread}}的参数默认以\textbf{值传递}方式传入。如果需要引用传递，\allowbreak{}必须使用\texttt{\seqsplit{std::ref}}或\texttt{\seqsplit{std::cref}}。
...
% ───────────────────────────────────────────
\chapter[协程机制深入：\texttt{promise\_type}与\texttt{coroutine\_handle}]{协程机制深入：\texttt{\seqsplit{promise\_type}}与\texttt{\seqsplit{coroutine\_handle}}}
% ───────────────────────────────────────────
...
\texttt{\seqsplit{this\_thread::sync\_wait}}是最常用的sender消费者，阻塞当前线程直到sender完成：
...
\texttt{\seqsplit{std::execution}}与\cpp{}20协程可以无缝互操作——sender可以在协程中被\texttt{\seqsplit{co\_await}}，协程也可以产生sender。
...
通过让\texttt{\seqsplit{promise\_type}}继承\texttt{\seqsplit{with\_awaitable\_senders}}，\allowbreak{}协程可以直接\texttt{\seqsplit{co\_await}}任何sender，无需显式调用\texttt{as\_awaitable}：
...
错误通过\textbf{error通道}自动在sender管道中传播，直到被\texttt{\seqsplit{upon\_error}}或\texttt{\seqsplit{let\_error}}处理：
...
% \printindex  % 未使用 makeidx 宏包，暂无索引
