...
\texttt{\seqsplit{VCPKG\_TARGET\_ARCHITECTURE}}、\texttt{\seqsplit{VCPKG\_CRT\_LINKAGE}}、
\texttt{\seqsplit{VCPKG\_LIBRARY\_LINKAGE}} 等变量。Vcpkg 内置了 \texttt{x64-windows}、
...
（\texttt{\seqsplit{target\_link\_libraries}}），而 ExternalProject 需要手动处理导入。
...
\toprule
\textbf{特性} & \textbf{Conan} & \textbf{Vcpkg} & \textbf{Fetch\allowbreak Content} & \textbf{XMake} & \textbf{Git/SVN} \\
\midrule
描述格式     & conanfile.py  & vcpkg.json   & \seqsplit{CMakeLists.txt} & xmake.lua  & .gitmodules / svn 属性 \\
版本表达     & 范围 + 约束   & 基线 + 覆盖  & 精确 tag/commit & 范围 + 精确 & 精确 commit/revision \\
Lock 文件    & conan.lock    & vcpkg.json 基线 & \seqsplit{package-lock.cmake} & xmake-repo 锁 & 天然锁定 \\
...
CMake 集成   & Generator 文件 & Toolchain 文件 & 原生内置         & 需切换构建系统 & \seqsplit{add\_subdirectory} \\
其他构建系统  & 支持 \seqsplit{Meson/Autotools} & 有限支持 & 仅 CMake        & 自带构建系统  & 取决于子目录结构 \\
...
自定义工具链  & profile 完整定义 & triplet +\allowbreak{} toolchain & 手动传入 & 内置 + 自定义  & 完全手动 \\
...
跨平台是 C++ 包管理的核心挑战。同一个项目需要在 Windows/\allowbreak Linux/\allowbreak macOS 上编译，
可能需要处理不同编译器（MSVC/\allowbreak GCC/\allowbreak Clang）、不同 C 标准库（glibc/\allowbreak musl/\allowbreak MSVCRT）、
不同图形后端（Win32/\allowbreak X11/\allowbreak Cocoa）等差异。
...
修改后通过 \texttt{\seqsplit{FetchContent\_Declare}} 指向本地路径或修改后的 Git URL。
...
\toprule
\textbf{场景} & \textbf{Conan} & \textbf{Vcpkg} & \textbf{Fetch\allowbreak Content} & \textbf{XMake} & \textbf{Git/SVN} \\
\midrule
指定平台     & 5 & 4 & 4 & 3 & 3 \\
跨平台       & 5 & 5 & 3 & 3 & 2 \\
系统定制     & 4 & 4 & 5 & 3 & 4 \\
